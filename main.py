...
async def update_guild_vc_roles(guild):
    ...
    if missing_roles:
        logger.warning(f"⚠️ Missing role IDs: {', '.join(str(r) for r in missing_roles)}")
        return
    ...
    for rank, row in enumerate(rows, 1):
        ...
        try:
            ...
            logger.warning(f"⚠️ No permission to update {member.name}")
        except Exception as e:
...
async def sync(ctx):
    try:
        ...
        await ctx.send(f"✅ Also synced {len(synced_global)} globally (up to 1 hour)")
    except Exception as e:
...
async def stats_slash(interaction: discord.Interaction, user: discord.Member = None):
    ...
    try:
        async with db_pool.acquire() as conn:
            ...
            rank_row = await conn.fetchrow('''
                SELECT COUNT(*) + 1 as rank FROM voice_time
                WHERE guild_id=$1 AND total_seconds > (
                    SELECT COALESCE(
                        (SELECT total_seconds FROM voice_time WHERE guild_id=$1 AND user_id=$2), 0
                    )
                )
            ''', interaction.guild.id, target_user.id)
    except Exception as e:
    ...
    embed = discord.Embed(title=f"🎤 {target_user.display_name}'s Streaming Stats", color=0x5865F2)
    embed.add_field(name="Total Time", value=f"**{row['total_seconds']/3600:.2f}** hours", inline=True)
    embed.add_field(name="Server Rank", value=f"**#{rank_row['rank'] if rank_row else 'N/A'}**", inline=True)
    embed.add_field(
        name="Last Updated",
...
async def resetstats_slash(interaction: discord.Interaction, user: discord.Member):
    ...
    try:
        ...
        if result == "DELETE 1":
            await interaction.followup.send(f"✅ Reset stats for {user.display_name}")
        else:
